...
var PrimeNumbersCalculator = WinJS.Class.define(function (numberOfWokers) {
    this.numberOfWorkers = numberOfWokers;
...
},
{
    ...
    setNumberOfWorkers: function (numberOfWorkers) {
        var newWorkersCount = numberOfWorkers- this.numberOfWorkers;
        if (newWorkersCount < 0) {
            return;
        }
        var newWorkers = this._initializeWebWorkers(newWorkersCount);
        
        for (var i = 0; i < newWorkers.length; i++) {
            this.freeWorkers.push(newWorkers[i]);
        }
    }
